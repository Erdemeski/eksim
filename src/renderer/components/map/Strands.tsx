...
interface StrandsProps {
  ...
  className?: string
  /**
   * true iken kare göndermez, WebGL bağlamı canlı kalır (LightRays.tsx'teki
   * `pausedRef.current || document.hidden` deseniyle birebir) — ActiveVideoBanner
   * bu bileşeni KALICI mount eder, yalnız bu prop'u aç/kapatır. Bu turun
   * kararlılık dersi: WebGL context'i tekrar tekrar yaratıp yok etmek yerine
   * canlı tutup görünürlüğü prop'la yönetmek (bkz. MagicRings/SilkBackground).
   */
  paused?: boolean
}

/**
 * reactbits Strands (JS+Tailwind varyantı) — birebir TS portu, shader/glass
 * mantığı DEĞİŞTİRİLMEDEN korunmuştur. Kaynak: kullanıcının sağladığı component
 * promptu ("npx shadcn@latest add @react-bits/Strands-JS-TW"). `ogl` zaten
 * proje bağımlılığı (LightRays.tsx) — yeni paket eklenmedi.
 */
export default function Strands({
  ...
  className = '',
  paused = false
}: StrandsProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const update = (t: number): void => {
      ...
      try {
        ...
        console.warn('Strands WebGL render hatası:', error)
      }
    }
    animateId = requestAnimationFrame(update)
    ...
    return () => {
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.warn('Strands WebGL temizleme hatası:', error)
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  return <div ref={ctnDom} className={`relative h-full w-full bg-transparent ${className}`} />
}
